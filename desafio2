...
def depositar(saldo, valor, extrato, /):
    if valor > 0:
        ...
        extrato += f"{now_sp.strftime('%d/%m/%Y %H:%M:%S')} - Depósito de R$ {valor:.2f}\n"
...
def sacar(*, saldo, valor, extrato, limite, numero_saques, limite_saques):
    ...
    if excedeu_saldo:
        ...
        sp_tz = pytz.timezone('America/Sao_Paulo')
        now_sp = datetime.now(sp_tz)
        extrato += f"{now_sp.strftime('%d/%m/%Y %H:%M:%S')} - Saque de R$ {valor:.2f}\n"
        numero_saques += 1
        numero_saques += 1
...
def exibir_extrato(saldo, /, *, extrato):
    print("\n------------= EXTRATO =------------")
    # Exibe as transações com data e hora
